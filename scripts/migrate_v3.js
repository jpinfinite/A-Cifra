...
async function processFile(filePath) {
    ...
    try {
        ...
        console.error(`Error parsing frontmatter for ${path.basename(filePath)}:`, e.message);
        // Attempt simple fix for the specific known issue (unquoted title with colon)
        // Regex to find title: ... : ... and quote it? Too risky.
        return;
    }
        ...
        if (path.resolve(sourcePath) !== path.resolve(newDestPath)) {
             try {
                await sharp(sourcePath)
                 ...
                 console.error(`Sharp error on ${slug}: ${imgErr.message}`);
                 return;
             }
        }
